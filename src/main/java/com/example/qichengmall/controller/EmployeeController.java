...
public class EmployeeController {
    ...
    public R<Employee> login(HttpServletRequest request,@RequestBody Employee employee){
        ...
        //2、根据页面提交的用户名username查询数据库employee
        LambdaQueryWrapper<Employee> queryWrapper = new LambdaQueryWrapper<>();
        queryWrapper.eq(Employee::getUsername,employee.getUsername());
        Employee emp = employeeService.getOne(queryWrapper);
        //todo 代码写的比较随意，后面可以细化一下
        //3、如果没有查询到则是商户，查询数据库business
        if(emp == null){
            LambdaQueryWrapper<Business> businessLambdaQueryWrapper = new LambdaQueryWrapper<>();
            businessLambdaQueryWrapper.eq(Business::getUsername,employee.getUsername());
